// --- Memory Brain formal injection adapter service (v0.6.0) ---
// 编排唯一正式记忆入口 adapter 演练；不接 sendMessage/getAiReply/正式提示词语义。
(function registerMemoryBrainFormalInjectionAdapterService(global) {
    ...
    function record(label, data, level) {
        ...
        if (service && typeof service.recordOperation === 'function') return service.recordOperation({ source: 'features/memoryBrain/formalInjectionAdapterService', sourceModule: 'features/memoryBrain/formalInjectionAdapterService', label, level: level || 'event', data: data || {} });
        return null;
    }
    ...
    function runFormalInjectionAdapter(options = {}) {
        const state = rootState(options);
        ...
        record('记忆脑正式注入 adapter 输入', input);
        const report = core().buildFormalInjectionAdapterPackage({ query, snapshot, ownerState: snapshot.ownerState, legacyBlock, legacyOwner });
        const result = api().appendFormalInjectionAdapterRun({ input, report }, { state });
        record('记忆脑正式注入 adapter 应用结果', { reportId: result.report && result.report.id, runId: result.run && result.run.id, finalOwner: result.report && result.report.final && result.report.final.owner, cutoverGate: result.report && result.report.owner && result.report.owner.cutoverGate, issueCount: result.report && result.report.issues && result.report.issues.length, formalPromptInjection: false, promptHooked: false }, 'success');
        return result;
    }
    function rollbackLatestFormalInjectionAdapterBatch(options = {}) {
        const result = api().rollbackLatestFormalInjectionAdapterBatch(options);
        record('记忆脑正式注入 adapter 批次回滚', { result, formalPromptInjection: false }, 'success');
        return result;
    }
    function getFormalInjectionAdapterCards(options = {}) {
        const snapshot = api().getFormalInjectionAdapterSnapshot(options);
        const compact = app.core.memoryBrain.formalInjectionAdapterSemantics.compactFormalInjectionAdapterForList;
        return {
    ...
    feature.formalInjectionAdapterService = { runFormalInjectionAdapter, rollbackLatestFormalInjectionAdapterBatch, getFormalInjectionAdapterCards, record };
})(window);
