...
// 数据保存与加载
const legacySaveDataImpl = async () => {
    // 存储配额预检
 ...
 * 失败时自动降级为全量 saveData
 */
const legacySaveCharacterImpl = async (characterId) => {
 ...
 * 失败时自动降级为全量 saveData
 */
const legacySaveGroupImpl = async (groupId) => {
 ...
 */
const legacySaveGlobalSettingsImpl = async () => {
    try {
        const allSettingKeys = [...globalSettingKeys, 'worldBookCategoryOrder'];
...
const loadData = async () => {
    ...
    const defaultValue = window.OwoApp.app.state.globalSettingsDefaults.createDefaultGlobalSettings();
    globalSettingKeys.forEach(key => {
        db[key] = settings[key] !== undefined
            ? settings[key]
            : (defaultValue[key] !== undefined ? JSON.parse(JSON.stringify(defaultValue[key])) : undefined);
    });
...
storageRepository.setLegacyWriters({
    saveData: legacySaveDataImpl,
    saveCharacter: legacySaveCharacterImpl,
    saveGroup: legacySaveGroupImpl,
    saveGlobalSettings: legacySaveGlobalSettingsImpl,
    loadData
}, {
    state: 'legacy-owner',
    owner: 'js/db.js',
    note: 'V3: db.js private writer 仍是唯一写入实现；repository 只提供统一公开入口和保存状态'
});
...
window.OwoApp.compat.expose('saveGlobalSettings', saveGlobalSettings, {
    ...
    note: 'V3: 旧 window.saveGlobalSettings 只保留兼容出口'
});
