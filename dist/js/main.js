...
const init = async () => {
    ...
    // 保存状态由 platform/storage/repository 统一追踪，避免 main.js 再包装 window.saveData 造成第二层路径。
    const storageRepository = window.OwoApp
        && window.OwoApp.platform
        && window.OwoApp.platform.storage
        && window.OwoApp.platform.storage.repository;

    // 用户关闭或刷新页面时，如有未完成保存则弹出提示
    window.addEventListener('beforeunload', (e) => {
        if (storageRepository && storageRepository.isSaving()) {
            e.preventDefault();
