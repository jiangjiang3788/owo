...
(function registerAiRequestTraceStore(global) {
    ...
    const MAX_PAYLOAD_TEXT_CHARS = 1200;
    let sequence = 0;
    const traces = [];
    const listeners = [];
    ...
    function sanitizeTracePayload(value, keyHint, depth) {
        const key = String(keyHint || '');
        const level = Number(depth || 0);
        if (level > 8) return '[omitted: max depth]';
        if (value === undefined || value === null) return value;
        if (/authorization|api[-_]?key|apikey|token|secret|password/i.test(key)) return '***redacted***';
        if (typeof value === 'string') {
            if (/^data:[^;]+;base64,/i.test(value)) return `[binary omitted:${value.length} chars]`;
            const max = /content|prompt|message|text|input|output|body/i.test(key) ? MAX_PAYLOAD_TEXT_CHARS : 4000;
            return value.length > max ? value.slice(0, max) + `... [truncated ${value.length - max} chars]` : value;
        }
        if (typeof value === 'number' || typeof value === 'boolean') return value;
        if (Array.isArray(value)) return value.slice(0, 80).map(item => sanitizeTracePayload(item, key, level + 1));
        if (typeof value === 'object') {
            const output = {};
            Object.keys(value).slice(0, 120).forEach(childKey => {
                output[childKey] = sanitizeTracePayload(value[childKey], childKey, level + 1);
            });
            return output;
        }
        return String(value);
    ...
    function extractRequestBody(fetchOptions, explicitBody) {
        if (explicitBody !== undefined) return sanitizeTracePayload(explicitBody, 'requestBody', 0);
        const body = fetchOptions && fetchOptions.body;
        if (body === undefined || body === null) return undefined;
        if (typeof body !== 'string') return '[non-string body]';
        try { return sanitizeTracePayload(JSON.parse(body), 'requestBody', 0); } catch (error) { return sanitizeTracePayload(body, 'requestBody', 0); }
    }
    ...
    function recordRequestStart(meta = {}) {
        ...
        return upsertTrace({
            ...
            taskType: meta.taskType || '', taskId: meta.taskId || '', routeId: meta.routeId || '', fallbackIndex: Number(meta.fallbackIndex || 0),
    ...
    function recordRequestSuccess(traceId, payload = {}) {
        ...
        if (payload.responseText !== undefined) {
            ...
            const redactedJson = sanitizeTracePayload(redactProviderPrivateFields(parsed), 'responseJson', 0);
            trace.responseBodyText = redactedJson ? JSON.stringify(redactedJson) : sanitizeTracePayload(limited.text, 'responseBodyText', 0);
    ...
    function recordDiagnostic(meta = {}) {
        ...
        Object.assign(trace, {
            ...
            requestBody: sanitizeTracePayload(meta.requestBody, 'requestBody', 0), diagnostic: sanitizeTracePayload(meta.diagnostic, 'diagnostic', 0), responseJson: sanitizeTracePayload(meta.responseJson, 'responseJson', 0),
    ...
    function recordConsoleEvent(meta = {}) {
        ...
        Object.assign(trace, {
            ...
            requestBody: sanitizeTracePayload(meta.requestBody, 'requestBody', 0), responseJson: sanitizeTracePayload(meta.responseJson, 'responseJson', 0),
            ...
            diagnostic: sanitizeTracePayload(meta.diagnostic, 'diagnostic', 0), event: sanitizeTracePayload(meta.event, 'event', 0), message: sanitizeTracePayload(meta.message, 'message', 0), content: meta.content !== undefined ? sanitizeTracePayload(String(meta.content), 'content', 0) : undefined,
    ...
    function recordAiResponseBatch(meta = {}) {
        ...
        const trace = recordConsoleEvent({
            ...
            responseJson: sanitizeTracePayload(redactProviderPrivateFields({
                usage: batch.usage,
    ...
    ai.requestTraceStore = { sanitizeTracePayload, trackedFetch, recordRequestStart, recordRequestSuccess, recordRequestFailure, recordDiagnostic, recordConsoleEvent, recordConversationEvent, recordAiResponseBatch, recordOperation, recordErrorEvent, getRecentTraces, clearTraces, subscribe, formatTraceForCopy, getMaxTraceCount: () => MAX_TRACE_COUNT };
