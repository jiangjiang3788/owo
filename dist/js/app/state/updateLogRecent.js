...
const updateLogRecent = [
    { version: "0.9.2", date: "2026-07-10", notes: ["私聊 Runtime 纵向切片：新增 legacy/shadow/unified 三态切换，三种模式复用同一个 Prompt 与 Provider Request；shadow 只做 dry-run preflight，不重复请求；文档统一收敛到 docs/。"] },
    { version: "0.9.1", date: "2026-07-10", notes: ["恢复安全基线：备份 checksum、导入前校验、Dexie 原子事务、失败回滚、脚本加载清单、Task 契约加固和 Trace 脱敏。"] },
    { version: "0.9.0", date: "2026-07-10", notes: ["统一 AI Task Runtime 与模型路由：新增 canonical task、Provider Account、Model Profile、Task Route 和故障回退；主聊天与旧通用 AI 请求先经过统一 Runtime，保留现有 Prompt、世界书、日记、档案和向量行为。"] },
