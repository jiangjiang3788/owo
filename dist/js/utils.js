...
const getRandomValue = window.OwoApp.shared.utils.getRandomValue;

// 图片压缩工具
// @compat canonical: OwoApp.platform.browser.compressImage
const compressImage = window.OwoApp.platform.browser.compressImage;

// 数字补零
// @compat canonical: OwoApp.shared.utils.pad
const pad = window.OwoApp.shared.utils.pad;

// 获取指定时区的当前当地时间
// @compat canonical: OwoApp.shared.utils.getLocalTimeInTimezone
const getLocalTimeInTimezone = window.OwoApp.shared.utils.getLocalTimeInTimezone;
...
// UUID 生成器
// @compat canonical: OwoApp.shared.utils.generateUUID
...
// @compat canonical: OwoApp.platform.browser.showSystemNotification
...
const formatTimeDivider = window.OwoApp.shared.utils.formatTimeDivider;

// 格式化时间戳 YYYY-MM-DD HH:MM:SS
// @compat canonical: OwoApp.shared.utils.getFormattedTimestamp
const getFormattedTimestamp = window.OwoApp.shared.utils.getFormattedTimestamp;
...
// 旧功能调用统一 AI Runtime 时的任务类型推断。
function inferLegacyAiTaskType(settings, endpoint) {
    const app = window.OwoApp || {};
    const contracts = app.core && app.core.ai && app.core.ai.taskContracts;
    let sourceKey = '';
    if (typeof db !== 'undefined' && db) {
        if (settings === db.summaryApiSettings) sourceKey = 'summaryApiSettings';
        else if (settings === db.backgroundApiSettings) sourceKey = 'backgroundApiSettings';
        else if (settings === db.imageRecognitionApiSettings) sourceKey = 'imageRecognitionApiSettings';
        else if (settings === db.vectorApiSettings) sourceKey = 'vectorApiSettings';
        else if (settings === db.apiSettings) sourceKey = 'apiSettings';
    }
    if (contracts && typeof contracts.inferLegacyTaskType === 'function') {
        return contracts.inferLegacyTaskType({ sourceKey, source: endpoint || '' });
    }
    return sourceKey === 'summaryApiSettings' ? 'system.summarize' : 'system.generic';
}
...
async function fetchAiResponse(settings, requestBody, headers, endpoint, forceStream = false) {
    ...
    const app = window.OwoApp || {};
    const runtime = app.features && app.features.aiRuntime && app.features.aiRuntime.publicApi;
    const requestTraceStore = app.platform && app.platform.ai ? app.platform.ai.requestTraceStore : null;
    const providerRequest = { endpoint, fetchOptions, requestBody };
    const taskType = inferLegacyAiTaskType(settings, endpoint);
    const response = runtime && typeof runtime.executeProviderRequest === 'function'
        ? await runtime.executeProviderRequest({
            taskType,
            providerRequest,
            settings,
            provider,
        ...
        : requestTraceStore && typeof requestTraceStore.trackedFetch === 'function'
            ? await requestTraceStore.trackedFetch(providerRequest, {
                source: 'utils.fetchAiResponse.compat',
                label: '通用 AI 请求',
                provider,
                model: settings && settings.model,
                stream: streamEnabled,
...
window.OwoApp.compat.expose('compressImage', compressImage, {
    state: 'canonical',
    owner: 'OwoApp.platform.browser.compressImage',
    note: 'V2: 旧 window.compressImage 只保留兼容出口'
});
window.OwoApp.compat.expose('generateUUID', generateUUID, {
    state: 'canonical',
    owner: 'OwoApp.shared.utils.generateUUID',
    note: 'V2: 旧 window.generateUUID 只保留兼容出口'
});
window.OwoApp.compat.expose('formatTimeDivider', formatTimeDivider, {
    state: 'canonical',
    owner: 'OwoApp.shared.utils.formatTimeDivider',
    note: 'V2: 旧 window.formatTimeDivider 只保留兼容出口'
});
window.OwoApp.compat.expose('getFormattedTimestamp', getFormattedTimestamp, {
    state: 'canonical',
    owner: 'OwoApp.shared.utils.getFormattedTimestamp',
    note: 'V2: 旧 window.getFormattedTimestamp 只保留兼容出口'
});
window.OwoApp.compat.expose('showSystemNotification', showSystemNotification, {
    state: 'canonical',
    owner: 'OwoApp.platform.browser.showSystemNotification',
    note: 'V2: 旧 window.showSystemNotification 只保留兼容出口'
});
window.OwoApp.compat.expose('getRandomValue', getRandomValue, {
    state: 'canonical',
    owner: 'OwoApp.shared.utils.getRandomValue',
    note: 'V2: 旧 window.getRandomValue 只保留兼容出口'
});
window.OwoApp.compat.expose('pad', pad, {
    state: 'canonical',
    owner: 'OwoApp.shared.utils.pad',
    note: 'V1/V2: 旧 window.pad 只保留兼容出口'
});
window.OwoApp.compat.expose('formatTimeGap', formatTimeGap, {
    state: 'canonical',
    owner: 'OwoApp.shared.utils.formatTimeGap',
    note: 'V2: 旧 window.formatTimeGap 只保留兼容出口'
});
window.OwoApp.compat.expose('getLocalTimeInTimezone', getLocalTimeInTimezone, {
    state: 'canonical',
    owner: 'OwoApp.shared.utils.getLocalTimeInTimezone',
...
window.OwoApp.compat.expose('showAppConfirmDialog', showAppConfirmDialog, {
    ...
    note: 'V2: 旧 window.showAppConfirmDialog 只保留兼容出口'
});
