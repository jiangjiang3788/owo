...
// v0.9.0: Provider fetch 统一经 AI Task Runtime；本文件继续负责旧聊天流解析和业务编排。
...
const chatAiTaskRuntime = window.OwoApp.features.aiRuntime.publicApi;
const chatAiConversationRuntime = window.OwoApp.features.chatRuntime && window.OwoApp.features.chatRuntime.publicApi;

async function executeChatAiProviderRequest(taskType, providerRequest, meta) {
    const details = meta || {};
    if (!chatAiTaskRuntime || typeof chatAiTaskRuntime.executeProviderRequest !== 'function') {
        throw new Error('AI Task Runtime 未加载，无法执行聊天请求');
    }
    return chatAiTaskRuntime.executeProviderRequest({
        taskType,
        providerRequest,
        ...
        source: details.source || 'chat_ai',
        label: details.label || taskType,
...
async function generateImageDescription(msg, chat, apiConfig) {
    ...
    try {
        ...
        const response = await executeChatAiProviderRequest('image.describe', providerRequest, {
            source: 'chat_ai.generateImageDescription',
            label: '图片自动描述',
            settings: normalizedApiConfig,
            provider,
            model,
            stream: false,
            requestBody
...
async function getAiReply(chatId, chatType, isBackground = false, isSummary = false, isCharBlockedMonologue = false, isPhoneControlRevokeAttempt = false) {
    ...
    try {
        ...
        const runtimeTaskType = isSummary ? 'conversation.summary' : (isBackground ? 'conversation.background' : 'conversation.reply');
        const conversationRequest = {
            ...
            chatId,
            chatType,
            character: chatType === 'private' ? chat : null,
            diagnostics: {
                historyMessageCount: historySlice.length,
                systemPromptChars: typeof systemPrompt === 'string' ? systemPrompt.length : 0,
                requestBuiltBy: 'chat_ai.single-builder'
            }
        };
        const response = chatType === 'private' && chatAiConversationRuntime && typeof chatAiConversationRuntime.executePreparedRequest === 'function'
            ? await chatAiConversationRuntime.executePreparedRequest(conversationRequest)
            : await executeChatAiProviderRequest(runtimeTaskType, providerRequest, conversationRequest);
        if (!response.ok) {
...
async function getCallReply(chat, callType, callContext, onStreamUpdate) {
    ...
    try {
        const response = await executeChatAiProviderRequest('conversation.video_call', providerRequest, {
...
async function generateCallSummary(chat, callContext) {
    ...
    try {
        const response = await executeChatAiProviderRequest('conversation.summary', providerRequest, {
