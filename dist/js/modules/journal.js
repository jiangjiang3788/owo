...
// v0.9.3 canonical AI executor for journal generation/merge. This service never writes journal data.
const journalRuntime = window.OwoApp.features.journalRuntime.publicApi;
...
function setupMemoryJournalScreen() {
    ...
    async function mergeJournals(journalIds) {
        ...
        try {
            const taskResult = await journalRuntime.executeJournalTask({
                taskType: 'journal.merge',
                state: db,
                prompt: summaryPrompt,
                source: 'journal.merge',
                label: '日记合并',
                legacySettingsKey: 'apiSettings',
                temperature: 0.7
            });
            const journalData = taskResult.value;
...
async function generateJournal(start, end, includeFavorited = false, silent = false, nodeInfo = null, options = {}) {
    ...
    try {
        ...
        let summaryPrompt = '';
        let journalTaskType = 'journal.generate';
        ...
        if (currentChatType === 'group') {
            journalTaskType = 'journal.summarize';
            // 群聊逻辑
            // 收集关联的 + 全局的世界书（去重）
...
聊天记录如下：\n\n---\n${(() => {
        ...
        const taskResult = await journalRuntime.executeJournalTask({
            ...
            state: db,
            prompt: summaryPrompt,
            source: 'journal.generate',
            label: journalTaskType === 'journal.summarize' ? '日记摘要生成' : '日记生成',
            legacySettingsKey: 'summaryApiSettings',
            temperature: 0.7
        });
        const journalData = taskResult.value;
