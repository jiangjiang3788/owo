...
注入质量测试集：用自动测试输入和自定义测试输入检查 Memory Brain shadow 召回是否漏召回、错召回、重复召回或过长。

UI 收敛：默认进入清爽模式，隐藏后厨式阈值、切片、队列等专家控件；需要时可切换专家模式。用户只看目标、结果和下一步，技术细节仍保留在专家模式和控制台。

## v0.6.6：正式默认开启候选 / 手动开启前 gate

Memory Brain 默认可以作为未来正式 owner 候选参与报告、质量测试和 shadow 演练；用户可运行开启前 gate 查看是否仍有可信、质量或 owner 安全问题。

本阶段仍不接正式 prompt，最终正式 owner 继续保持 legacy，直到 `blocked-until-v0.9` 总门禁解除。
