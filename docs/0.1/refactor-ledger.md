...
- 产品/功能迭代版本改为 `v0.2.1`；历史 `V27/V37/V38` 继续作为 gate 编号保留。
- 补齐 `docs/*` 根路径 gate 文档入口，避免后续新功能开发时把旧基线失败误判为新变更失败。
- `js/app/state/staticConfigBase.js` 将 `appVersion` 调整为 `0.2.1`，并新增 `appName = "章鱼喷墨机"` 供 UI/设置页后续复用。
- 本版只做基线和版本标识，不实现悬浮球、请求控制台、表格记忆诊断、聊天框重排或设置第二页。


## v0.2.2：请求追踪底座和控制台 MVP

- 新增 `js/platform/ai/requestTraceStore.js`，作为 AI/API 请求可观测 canonical owner。
- 新增 `js/features/debugConsole/service.js`、`view.js`、`public.js`，提供右下角“请求”控制台、单条复制、全部复制和清空能力。
- 新增 `css/modules/debug_console.css` 并登记 `tools/css-ownership-map.json`，避免样式继续堆进 chat/settings legacy CSS。
- 接入主聊天、自动总结、表格记忆等 `fetchAiResponse` 链路，以及 chat_ai 直接请求、embedding、生图、识图和模型列表请求。
- 请求体和模型返回内容会记录；密钥类 header 与 URL key 参数默认打码。


## v0.2.3：表格记忆 XML 返回诊断版

- 新增 `js/core/memory/tableUpdateXmlSemantics.js`，作为表格记忆模型返回清洗和诊断对象的 core owner。
- 新增 `js/features/memoryTable/updateDiagnosticsService.js`，负责结合当前模板、表格、字段、锁定状态和 rows 行数据验证 `<memory_updates>`。
- `js/modules/memory_table.js` 保留 legacy 编排，但 XML 解析入口改为调用 `assertParsedMemoryUpdates()`，失败时不再只有一句“解析失败”。
- `js/platform/ai/requestTraceStore.js` 新增 `recordDiagnostic()`，让非网络诊断也能进入请求控制台复制。
- `docs/smoke-memory.md` 和 `tools/memory-regression-gate.js` 增加 `MEM-TABLE-XML-01` 与坏格式样例检查。


## v0.2.4 — Chat panel and home settings page

- 新增 `js/features/home/homeAppCatalog.js` 作为主屏 app 分组 owner，只存元数据，不访问 DOM / db / fetch。
- `js/ui.js` 继续作为 legacy home shell，消费 home catalog 渲染第一页高频入口和第二页设置入口。
- `js/features/chat/chatRoomScreenTemplate.js` 只调整展开面板按钮顺序，保留所有按钮 ID。
- `css/chat.css` 压缩展开面板高度、间距、图标尺寸和文字尺寸，使工具面板更接近微信紧凑样式。
- `css/layout.css` 收敛 dock 占位，设置类入口统一移到第二页并显示名称。
- 记录 v0.2.6 悬浮球补充需求：提示词入口、大尺寸可关闭面板、提示词真实换行展示。

## v0.2.5 — Model switch and GitHub cloud backup facades

- 新增 `js/features/settings/apiSettings/apiModelSwitchService.js`，作为主 API 模型候选收集和切换 owner。
- `js/features/settings/apiSettings/public.js` 暴露 `getCurrentMainModel()`、`listMainModels()`、`switchMainModel()`，供后续悬浮球快速切模型调用。
- 新增 `js/platform/storage/githubBackupAdapter.js`，GitHub API 状态检查、单文件上传、分片上传、最新备份查找和下载解压都归 platform adapter。
- 新增 `js/features/cloudBackup/service.js`、`public.js`，负责 GitHub 配置、本地自动备份判断、备份恢复用例编排和稳定出口。
- `js/modules/tutorial.js` 中 `window.GitHubMgr` 改为 compatibility shell，继续服务旧教程页按钮，但不再拥有上传/恢复核心逻辑。
- 后续 v0.2.6 悬浮球必须调用 `apiSettings.publicApi`、`cloudBackup.publicApi` 和 `debugConsole.publicApi`，不能直接访问教程页 DOM 或 `GitHubMgr`。


## v0.2.6 — Quick dock MVP

- Added `features/quickDock` as the owner of the floating quick dock.
- Quick dock calls model switching, cloud backup and debug console through public facades only.
- Added prompt panel in quick dock with real multiline textarea rendering and `\n` compatibility normalization.
- Enlarged debug console panel for full request/diagnostic copying.
- Moved “新增消息” next to edit-message format actions, after “插入动作”.
- Disabled startup update reminder by making `checkForUpdates()` only mark the current version as seen.

## v0.2.7 — Closing review and guard rails

- Reviewed v0.2.4–v0.2.6 changes for ownership drift and user-visible rough edges.
- Fully disabled automatic new-feature guide overlays by making `GuideSystem.check()` no-op by default; manual update log remains available in tutorial.
- Fixed GitHub contents API path handling in `githubBackupAdapter` by encoding each path segment rather than encoding the whole `repoPath` string.
- Added explicit hidden CSS rules for quick dock and request console panels.
- Adjusted quick dock prompt panel `×` to close directly and made opening request console collapse quick dock to avoid overlapping large panels.
- Hardened `cloudBackup.service` backup context with explicit `global.db` / `global.dexieDB` checks instead of undeclared fallback globals.
- Extended `tools/feature-integration-gate.js` to cover debugConsole/cloudBackup/quickDock public facades and quick dock load-order constraints.


## v0.2.8 — 请求控制台完全收进悬浮球

- 取消 debugConsole 的独立右下角“请求”悬浮按钮；请求入口只保留在 quickDock。
- debugConsole.view 改为嵌入式面板渲染 owner，新增 `renderEmbedded()` / `destroyEmbedded()`。
- quickDock.view 新增 `requests` activePanel，直接承载请求控制台 UI。
- feature-integration-gate 增加禁止独立请求入口的检查，避免后续回退到两个悬浮入口。



## v0.2.9 — docs 按 0.1 / 0.2 分卷

- 新增 `docs/README.md` 作为文档路由入口，明确 `docs/0.1`、`docs/0.2`、`docs/caifen` 和根路径 gate facade 的职责。
- 新增 `docs/0.1/README.md` 与 `docs/0.1/gate-index.md`，把历史 V2～V38.1 架构 gate 文档收进 0.1 导航。
- 新增 `docs/0.2/README.md` 与 `docs/0.2/release-plan.md`，把 v0.2.x 功能 release 文档作为当前 canonical owner。
- 根路径 `docs/release-plan.md` 改为 compatibility facade；gate 固定入口如 `docs/smoke-memory.md`、`docs/css-ownership.md` 不移动。
- `tools/arch-check.js` 增加 v0.2.9 文档路由检查，防止后续继续把新功能文档散落到根路径或继续使用 V39/V40 当产品版本。



## v0.2.10 — docs 版本线修正 + 快速切换模型紧凑化

- 修正 v0.2.9 对文档归档的表达：原历史架构文档归入 `docs/0.1/` 作为 canonical owner。
- 新增 `docs/other/README.md`，给非 0.1 历史线、非 0.2 release 线的长期说明预留位置。
- 根路径 `docs/caifen/` 仅保留 compatibility README，不继续承载 canonical 历史文档。
- 悬浮球快速切换模型只展示模型名，不再显示 `(newapi · 当前表单)`，并压缩 select/button 字号与间距。

## v0.2.12 docs root allowlist

- 按用户要求收口 `docs/` 根路径，只保留 `0.1`、`0.2`、`css-ownership.md`、`release-plan.md`、`smoke-memory.md`、`VERSIONING.md`、`README.md`。
- 移除 `docs/caifen/`、`docs/other/`、根路径 `release-v0.2.x-plan.md` 兼容副本、根路径 V27/V37/V38 兼容文档和 `docs/versioning.md`。
- 明确未来开启 `v0.3.x` 时再创建 `docs/0.3/`，当前不提前占位。

## v0.2.14 — App 入口合并收口

- Dock 第一个入口改为 `api-settings-screen`，并取消 `home-action-dock` 的半透明白色底板，让 Dock 图标尺寸与普通 app 图标一致。
- `features/home/homeAppCatalog` 不再把壁纸、自定义、白昼、夜间、教程、存储分析作为可见主页入口；这些旧功能入口被标记为合并到新 App。
- `appearance-settings-screen` 直接承载壁纸设置、主屏自定义和白昼/夜间模式，不再跳转 `wallpaper-screen` / `customize-screen`。
- `data-management-screen` 直接承载请求控制台、教程/备份内容和存储分析面板，不再跳转 `tutorial-screen` / `storage-analysis-screen`。
- `renderTutorialContent(container)` 支持指定容器渲染，服务数据管理 App 内嵌完整教程/备份设置。
- `setupCustomizeApp()` / `renderCustomizeForm()` 增加空容器保护，旧页面兼容壳不会在初始化时抛错。

## v0.2.15 — 统一控制台与 Dock 收口

- 首页不再展示数据管理、提示词、外观设置；这三个入口只保留在 Dock 栏，API 仍为 Dock 第一个入口。
- 新增 `platform/observability/traceStore` 作为控制台写入 facade，复用 `platform/ai/requestTraceStore` 实际存储。
- `addMessageBubble` 接入发送/回复 trace，用户消息、AI 回复、AI/API 请求、错误和诊断进入同一控制台。
- `quickDock` 不再渲染控制台面板，只调用 `dataManagement.publicApi.openConsole()`。
- `debugConsole` 保留渲染能力，作为数据管理 App 内部控制台组件；详情和复制均使用真实换行展示。

## v0.2.16 — 控制台宿主收口

- `features/quickDock` 重新作为控制台宿主：点击悬浮球“控制台”直接在悬浮球大面板里查看，不跳转数据管理页面。
- `features/debugConsole` 仍是唯一控制台 renderer；quickDock 只挂载 `renderEmbedded()`，不复制请求列表、详情、复制、清空逻辑。
- `features/dataManagement` 只保留控制台入口说明，避免数据管理页面再套一个完整控制台造成重复。
- `tools/feature-integration-gate.js` 更新为允许 `console` 面板、禁止旧 `requests` 面板和 debugConsole 独立浮动按钮。
